...
let width = window.innerWidth * 2; //windowの幅
...
var engine = Engine.create(container, {
  render: {
    ...
    options: {
      ...
      width: container_w, //canvasのwidth(横幅)
      height: container_h, //canvasのheight(高さ)
...
World.add(engine.world, [
  Bodies.rectangle(0, container_h, container_w * 2, 2, {
    isStatic: true, //固定する
    render: {
      fillStyle: "#fff", // 塗りつぶす色: CSSの記述法で指定
...
World.add(engine.world, [
  Bodies.rectangle(0, 0, 2, container_h * 2, {
    isStatic: true, //固定する
    render: {
      fillStyle: "#fff", // 塗りつぶす色: CSSの記述法で指定
      strokeStyle: "rgba(0, 0, 0, 0)", // 線の色: CSSの記述法で指定
...
World.add(engine.world, [
  Bodies.rectangle(container_w, 0, 2, container_h * 2, {
    isStatic: true, //固定する
    render: {
      fillStyle: "#fff", // 塗りつぶす色: CSSの記述法で指定
      strokeStyle: "rgba(0, 0, 0, 0)", // 線の色: CSSの記述法で指定
...
let x = getRandomParameter(container_w, 0);
let y = getRandomParameter(-1000, -500);

World.add(engine.world, [
  Bodies.circle(arrayX[0], arrayY[0], 111, {
    //ボールを追加
